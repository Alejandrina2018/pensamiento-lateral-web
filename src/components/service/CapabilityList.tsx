...
type CapabilityListProps = {
  ...
  /** 3 columns is for exactly-3-item lists (one per column, e.g. the
   * methodology blocks); 2 uses a flowing multi-column layout so an odd
...
export default function CapabilityList({ items, columns = 2 }: CapabilityListProps) {
  if (columns === 3) {
    return (
      <div className="grid gap-10 border-t border-sand pt-10 md:grid-cols-3 md:divide-x md:divide-sand">
        {items.map((item) => (
          <div key={item.name} className="flex flex-col gap-3 md:px-8 md:first:pl-0 md:last:pr-0">
            ...
            <p className="leading-relaxed text-slate/80">{item.body}</p>
  ...
  return (
    <div className="border-t border-sand pt-10 md:columns-2 md:gap-10">
      {items.map((item) => (
